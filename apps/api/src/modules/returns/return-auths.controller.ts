...
export class ReturnAuthsController {
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Update return authorization' })
  @ApiParam({ name: 'id', description: 'Return Authorization ID' })
  ...
  @Post(':id/receive')
  @ApiOperation({ summary: 'Mark return authorization as received' })
  async receive(@Param('id') id: string) {
    return this.service.receiveReturnAuth(id);
  }
}
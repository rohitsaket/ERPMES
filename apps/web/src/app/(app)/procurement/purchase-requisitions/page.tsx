...
import type { PaginatedResponse, PurchaseRequisition } from "@/lib/api/types";
                  ...
                  <tbody>
                    {data.data.map((pr) => (
                      <tr key={pr.id} className="border-b last:border-0 hover:bg-muted/50">
                        <td className="py-3 font-mono text-xs">{pr.id.slice(0, 8)}...</td>
                        <td className="py-3">{pr.lines.length}</td>
                        <td className="py-3"><Badge className={statusColors[pr.status] || "bg-gray-100"}>{pr.status}</Badge></td>
                        <td className="py-3 text-muted-foreground">{new Date(pr.createdAt).toLocaleDateString()}</td>
                        <td className="py-3"><Link href={`/procurement/purchase-requisitions/${pr.id}`}><Button variant="ghost" size="sm"><Eye className="h-4 w-4" /></Button></Link></td>
                      </tr>
                    ))}
                  </tbody>
                </table>

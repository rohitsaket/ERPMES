...
export default function VendorsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.get<{ data: Vendor[]; meta: { page: number; limit: number; total: number; totalPages: number } }>("/vendors", { search, page, limit: pageSize }),
  ...
  if (error) return <AppShell><div className="text-center py-20 text-red-600">Failed to load vendors</div></AppShell>;
    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button disabled title="Vendor creation is not implemented yet"><Plus className="mr-2 h-4 w-4" />New Vendor</Button>
        </div>
                    ...
                    <tbody>
                      {vendors.map((vendor) => (
                        <tr key={vendor.id} className="border-b last:border-0 hover:bg-muted/50">
                          <td className="py-3 font-mono">{vendor.code}</td>
                          <td className="py-3 font-medium">{vendor.name}</td>
                          <td className="py-3">
                            <Badge variant="secondary" className={statusColors[vendor.status?.toUpperCase?.() ?? "ACTIVE"] || "bg-gray-100 text-gray-700"}>

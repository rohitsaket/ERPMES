...
import type { PaginatedResponse, SalesOrder } from "@/lib/api/types";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Plus, Search, Eye, ShoppingCart } from "lucide-react";
import { NewOrderModal } from "@/components/sales/new-order-modal";
...
export default function SalesOrdersPage() {
  return <AuthenticatedSalesOrdersPage />;
}

function AuthenticatedSalesOrdersPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<SalesOrder>>({
    ...
    queryFn: () => api.get("/sales-orders", { page, limit: 20, search }),
  });
                  ...
                  <tbody>
                    {data.data.map((order) => (
                      <tr key={order.id} className="border-b last:border-0 hover:bg-muted/50">
                        ...
                        <td className="py-3">{order.customer?.name || order.customerId.slice(0, 8)}</td>
                        <td className="py-3">
